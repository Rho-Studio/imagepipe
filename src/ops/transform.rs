        ...
        match img {
            ImageSource::Raw(img) => {
                let (rotation, fliph, flipv) = match img.orientation {
                    Orientation::Normal | Orientation::Unknown => (Rotation::Normal, false, false),
            ...
            ImageSource::Other(_) => OpTransform {
...
mod tests {
    ...
    fn rotate_flip_x() {
        ...
        assert_eq!(
            rotate_buffer(&F.clone(), &Orientation::HorizontalFlip),
            output
        );
    }
    ...
    fn rotate_flip_y() {
        let output = OpBuffer::from_rgb_str_vec(vec![
            "        ", " GG     ", " GG     ", " BBBB   ", " GG     ", " RRRRRR ", "        ",
        ]);
        assert_eq!(
            rotate_buffer(&F.clone(), &Orientation::VerticalFlip),
            output
        );
    }
