...
fn main() {
    ...
    let outfile = if args.len() > 2 { &args[2] } else { &filejpg };
    println!("Loading file \"{}\" and saving it as \"{}\"", file, outfile);

    /*
      let from_time = Instant::now();
      let image = match rawloader2::decode_file(file) {
        Ok(val) => val,
    ...
    */
    ...
    let decoded = match imagepipe2::simple_decode_8bit(file, 0, 0) {
        Ok(img) => img,
        Err(e) => error(&e),
    };
    ...
    let uf = match File::create(outfile) {
        Ok(val) => val,
        Err(e) => error(format!("Error: {}", e).as_ref()),
    };
