...
impl OpBuffer {
    ...
    {
        self.data
            .par_chunks_mut(self.width * self.colors)
            .enumerate()
            .for_each(|(row, line)| {
                closure(line, row);
            });
        ...
        F: Fn(&mut [f32], &[f32]) + Sync,
    {
        ...
        out.data
            .par_chunks_mut(out.width * out.colors)
            .enumerate()
            .for_each(|(row, line)| {
    ...
    pub fn transform(
        ...
        let newdata = crate::scaling::transform_buffer(
            &self.data,
            self.width,
            self.height,
            topleft,
    ...
    pub fn from_rgb_str_vec(data: Vec<&str>) -> OpBuffer {
        let width = data
            .first()
            .expect("Invalid data for rgb helper function")
            .len();
        let height = data.len();
        let colors = 3;
