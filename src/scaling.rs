...
fn calculate_scaling_total(
    ...
    if maxwidth == 0 && maxheight == 0 {
        ...
        let xscale = if maxwidth == 0 {
            1.0
        } else {
            width as f32 / maxwidth as f32
        };
        let yscale = if maxheight == 0 {
            1.0
        } else {
            height as f32 / maxheight as f32
        };
    ...
    T: Sync + Send,
{
    ...
    out.par_chunks_exact_mut(nwidth * components)
        .enumerate()
        .for_each(|(row, line)| {
            ...
            for col in 0..nwidth {
                let from_x = cmp::min(
                    width - 1,
                    (from_x + (skip_x_x * col as f32)).floor() as usize,
                );
                let to_x = cmp::min(
                    width - 1,
                    (to_x + (skip_x_x * (col + 1) as f32)).floor() as usize,
                );
                let from_y = cmp::min(
                    height - 1,
                    (from_y + (skip_x_y * col as f32)).floor() as usize,
                );
                let to_y = cmp::min(
                    height - 1,
                    (to_y + (skip_x_y * (col + 1) as f32)).floor() as usize,
                );
...
pub fn scaled_demosaic(cfa: CFA, buf: &OpBuffer, nwidth: usize, nheight: usize) -> OpBuffer {
    ...
    log::debug!(
        "Doing a scaled demosaic from {}x{} to {}x{}",
        buf.width,
        buf.height,
        nwidth,
        nheight
    );
    let data = scale_down_buffer(
        &buf.data,
        buf.width,
        buf.height,
        nwidth,
